...
export function StampOutlined({ className, size = 24, color = 'currentColor' }: IconProps) {
  if (size % 8 !== 0) {
    ...
    >
      ...
      <defs>
        <clipPath id='clip0_115_5417'>
          <rect width='24' height='24' fill='white' transform='translate(0.316406 0.570801)' />
        </clipPath>
      </defs>
    </svg>
  );
...
export function StampFilled({ className, size = 24, color = 'currentColor' }: IconProps) {
  if (size % 8 !== 0) {
    console.warn(`The size prop (${size}) should be a multiple of 8.`);
  }
